"""Layered full-body House representative SVG (1200×2400) for 3D pipelines."""
...
from representatives_look import props_for  # noqa: E402
...
def _outline_wrap(inner: str, scale: float = 1.0) -> str:
    ...
    sw = OUTLINE_W / scale
    return (
        f'<g id="outlines" fill="none" stroke="{OUTLINE}" stroke-width="{sw}" '
...
def _split_head(look: Look, s: tuple) -> tuple[str, str, str]:
    ...
    mouth = (
        f'<path fill="{mouth_c}" d="M375 408 Q400 418 425 408 '
        f'Q420 412 400 414 Q380 412 375 408 Z"/>'
    )
...
def render_body(look: Look, s: tuple) -> str:
    ...
    return legs_trousers(
        look,
        trouser_color(look, COAT_COLORS.get(look.coat, COAT_COLORS["charcoal"])[0]),
    )
...
def generate_representative_svg(
    look: Look,
    record_name: str,
    primary_year: int,
    state_slug: str,
    ...
    desc = (
        f"Full-body portrait of {record_name}, U.S. Representative "
        f"(primary House service from {primary_year}, {state_slug.replace('-', ' ').title()}). "
...
{accessories}
  ...
  <title id="title">{record_name} — U.S. Representative ({primary_year})</title>
  <desc id="desc">{desc}</desc>
  <metadata>
    House collection v1. Coordinate space 1200×2400; inner figure scaled 1.5×2 from 800×1200 rig.
